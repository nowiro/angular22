...
| T004 | Wpięcie Galileo (ingest OTel) + scoring     | orchestrator | run w Galileo pokazuje koszt/latencję + score wyboru narzędzia (AC2, AC3)                                    | todo   | Opus 4.8     | T003       |
| T005 | Kanon docs — `docs/observability.md`        | docs         | enablement (ustawienia, endpoint, klucz przez env), mapa span→krok SDD (AC4)                                 | todo   | Gemini Flash | T002, T004 |
| T006 | Linki kanonu + run-log telemetria           | docs         | `AGENTS.md` + `mcp-usage` linkują kanon; szablon run-logu wskazuje źródło auto (AC5)                         | todo   | Gemini Flash | T005       |
| T007 | Smoke obserwowalności (zamiast e2e)         | reviewer     | end-to-end trace per krok SDD potwierdzony w backendzie; brak luk (AC1–AC3)                                  | todo   | Gemini Flash | T004       |
| T008 | Stack + security + analyze + verify         | orchestrator | off-stack/pinning go (`stack-guardian`); 0 sekretów (`security`); `/analyze` go; `pnpm verify` zielone (AC6) | todo   | Opus 4.8     | T005–T007  |
